...
The accepted implementation is on `codex/inbox-zero-operationalization` at `a32da3d5c1384853e88593deb5ccbcfa298a4867`. Local build, serial full Vitest, integration, Pester, worker tests, and static checks pass. The readiness checker is intentionally `pilot-only` because runtime variables and protected receipts are absent. The first implementation task must preserve this clean branch and turn the missing readiness values into an operator-run activation checklist, not bypass them.
