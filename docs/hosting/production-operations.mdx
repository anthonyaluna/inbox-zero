...
The preflight fails closed unless the checkout is clean, Node 24 and pnpm 11
...
`NEXT_PUBLIC_EMAIL_SEND_ENABLED=false`. The verification receipt contains only
timestamps, commit SHA, service states, and pass/fail codes. Do not include
...
test command, preflight, and `git diff --check`. Its staging values come only
from the protected `coastline-inbox-zero-staging` environment.
